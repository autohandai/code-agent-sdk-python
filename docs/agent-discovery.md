...
```python
agents = await sdk.supported_agents()
for agent in agents:
    print(agent.name, agent.source)
```
...
The returned `SupportedAgentInfo` extends the existing `AgentInfo` type.
The legacy `get_agents()` alias retains its command-list behavior; use
`supported_agents()` for the effective subagent registry.
