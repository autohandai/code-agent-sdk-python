...
@pytest.mark.asyncio
async def test_pause_automode_delegates_to_sdk() -> None:
    """Agent exposes autonomous execution pause."""
    sdk = MagicMock(spec=AutohandSDK)
    sdk.pause_automode = AsyncMock(return_value="paused")

    assert await Agent.from_sdk(sdk).pause_automode() == "paused"
    sdk.pause_automode.assert_awaited_once_with()


@pytest.mark.asyncio
async def test_resume_automode_delegates_to_sdk() -> None:
    """Agent exposes autonomous execution resume."""
    sdk = MagicMock(spec=AutohandSDK)
    sdk.resume_automode = AsyncMock(return_value="resumed")

    assert await Agent.from_sdk(sdk).resume_automode() == "resumed"
    sdk.resume_automode.assert_awaited_once_with()
